...
des_stat = training_data[["Weight","Membership(Days)"]].describe()
des_stat
...
trainers = training_data["Trainer"].unique()
trainers
...
#Convert the Membership Days into Weeks and add a column for the weeks
...
renamed_df = organized_df.rename(columns={"Membership(Days)":"Membership in Days",
...
file_path = 'D:\\2019 Spring Baruch\\Columbia Engineering\\Python\\books.csv'
...
books.columns
trim_books = books[['isbn', 'original_publication_year', 'original_title',
       'authors', 'ratings_1','ratings_2', 'ratings_3', 'ratings_4', 'ratings_5']]
trim_books.head(5)
...
#Rename the headers
renamed_books = trim_books.rename(columns={'isbn':'ISBN', 
...
renamed_books.head(5)


# In[30]:


#Push the renamed data to a new csv file in the same directory
renamed_books.to_csv("D:\\2019 Spring Baruch\\Columbia Engineering\\Python\\renamed-books.csv", index=False, header=True)


# In[33]:


raw_data = pd.read_csv("D:\\2019 Spring Baruch\\Columbia Engineering\\Python\\books_clean.csv", encoding = 'utf-8')
raw_data.head()
...
author_count = len(raw_data["Authors"].unique())

# Calculate the earliest/latest year a book was published
earliest_year = raw_data["Publication Year"].min()
latest_year = raw_data["Publication Year"].max()

# Calculate the total reviews for the entire dataset
total_reviews = raw_data["One Star Reviews"].sum() + raw_data["Two Star Reviews"].sum() + raw_data["Three Star Reviews"].sum() + raw_data["Four Star Reviews"].sum() + raw_data["Five Star Reviews"].sum()
